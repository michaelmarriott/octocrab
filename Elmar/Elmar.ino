...
#define NUM_LEDS_OF_ROWS 1
#define NUM_STRIPS 8
#define NUM_OF_SHORTSTRIPS 0

int divider = 2;
int minOscillator = 4;
int delayOnColorWipeRain = 50;
bool revesableOnColorWipeRain = true;
int twinkleAmounts = 2;
  ...
  switch (pattern){
    ...
    case 2:
      ...
      TheaterChaseRainbowSequenceWrapper(1); 
       if (patternCounter > patternPeriod/10) {
        pattern += 1;
        patternCounter = 0;
      } 
      break; 
    case 3: 
      mainDelay= patternSpeed;
      FireSequenceWrapper();
      if (patternCounter > patternPeriod*2) {
        pattern += 1;
        patternCounter = 0;
      } 
      break; 
    ...
    //chnage to have 5-8 run down, 1-4 run up.
      ChangeColourSequenceWrapper(20); //100 at 150 pixels, maybe run all the to tail![chnage color every 2nd time.  
       if (patternCounter > patternPeriod*4) {
        pattern += 1;
        patternCounter = 0;
      } 
      break;
    case 6:
        ...
        if (patternCounter > patternPeriod*8) {
          pattern += 1;
          patternCounter = 0;
        } 
        break;
...
#define PEAK_COLOR       CRGB(120,250,250)
...
#define DELTA_COLOR_UP   CRGB(1,6,6)
...
void addTwinkle( fract8 chanceOfGlitter) {
  if( random8() < chanceOfGlitter) {
    leds[ random16(NUM_STRIPS*NUM_LEDS_PER_STRIP) ] += RandomColor(0,120,125,255,125,255);
  }
}
...
void ChangeColourSequence(int minBright){
  ...
  if (bounceDirection == 1) {
    ...
    bright--;
    if (bright <= 30) {
      hue = Increment(hue, 85,175,11);
      sat = Increment(sat, 120, 254,22);
      bounceDirection = 0;
    }
  }
...
// Array of temperature readings at each simulation cell
  ...
  // Step 1.  Cool down every cell a little
    ...
    for( int j = 0; j < NUM_LEDS_PER_STRIP; j++) {
      ...
      CRGB color = ColorFromPalette( CRGBPalette16( CRGB::Black, CRGB::DarkTurquoise, CRGB::PaleTurquoise,  CRGB(125,250,250)), colorindex);
      int pixelnumber;
...
void ColorWipeRainSequence(CRGB color, bool allowReverse, uint8_t wait) {
 ...
 //random16_add_entropy(random());
  ...
  trailMax = Increment(trailMax,4*NUM_LEDS_OF_ROWS, 8*NUM_LEDS_OF_ROWS,2);
  apart = Increment(apart,3*NUM_LEDS_OF_ROWS, 8*NUM_LEDS_OF_ROWS,2);
  amountOfAparts = Increment(amountOfAparts,3*NUM_LEDS_OF_ROWS, 8*NUM_LEDS_OF_ROWS,4);
  ...
  if (loopCounter == 0 ) {
    ...
    for (int x = 0; x < NUM_STRIPS*NUM_LEDS_OF_ROWS; x++) {
      for (int y = 0; y < NUM_LEDS_PER_STRIP/NUM_LEDS_OF_ROWS; y++) {
        leds[(x * (NUM_LEDS_PER_STRIP/NUM_LEDS_OF_ROWS)) + (y)] = CRGB(2,5,25);
      }
    }
    FastLED.show(); // display this frame
    ...
    //Got Out
    for (int s = 0; s < NUM_STRIPS*NUM_LEDS_OF_ROWS; s++) {
      colorOC[s] = RandomColor(0,120,125,255,125,255);
    }
    FastLED.show();
  }
...
void OscialateComplexSequence(int minimun) {
  ...
  for (int strip = 0; strip < (NUM_STRIPS*NUM_LEDS_OF_ROWS); strip++) {
    if (rdms[strip][1] <= 0) {// if you get to zero
      ...
      colorOC[strip] = RandomColor(0,120,125,255,125,255);
    }
...
CRGB ChangeColor() {
  ...
  return RandomColor(0,120,125,255,125,255);
...
CRGB Wheel(byte WheelPos) {
  ...
  if (WheelPos < 85) {
    return CRGB(85 - WheelPos, 125, WheelPos * 3);
  }
  else if (WheelPos < 170) {
    WheelPos -= 85;
    return CRGB(0, WheelPos * 3, 255 - WheelPos );
  }
  else {
    WheelPos -= 170;
    return CRGB(WheelPos, 255 - WheelPos, 255);
  }
}

void SetListOfColors(CRGB listOfColors[14]){
  listOfColors[0] =CRGB(64, 224, 208);//64,224,208
  listOfColors[1] =CRGB(174, 238, 238);// 174,238,238
  listOfColors[2] =CRGB(72, 209, 204);//72,209,204
  listOfColors[3] =CRGB(0, 206, 209);//0,206,209
  listOfColors[4] =CRGB(0, 190, 140);//0,199,140
  listOfColors[5] =CRGB(3, 168, 158);//3,168,158
  listOfColors[6] =CRGB(0, 255, 255);//0,255,255
  listOfColors[7] =CRGB(245, 255, 255);//224,255,255
  listOfColors[8] =CRGB(0, 139, 139);//0,139,139
  listOfColors[9] =CRGB(127, 255, 212);//127,255,212
  listOfColors[10] =CRGB(102, 205, 170);//102,205,170
  listOfColors[11] =CRGB(32, 178, 170);//32,178,170
  listOfColors[12] =CRGB(0, 128, 128);//0,128,128
  listOfColors[13] =CRGB(32, 224, 208);//32,224,208
}
